package io.rescueranch.demodrivegradle.access;
...
public class CreateDrive {
    ...
    public CreateDrive(Credential creds){
        this.creds = creds;
    }
    ...
    public String createDrive() throws IOException {
        ...
        Date date = new Date();
        date.setTime(creds.getExpirationTimeMilliseconds());
        GoogleCredentials credentials = GoogleCredentials.newBuilder().setAccessToken(new AccessToken(creds.getAccessToken(), date)).build().createScoped(DriveScopes.all());
